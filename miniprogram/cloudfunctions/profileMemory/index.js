...
function aiPrompt(sources) {
  ...
  return [
    ...
    '必须具体到可以被否定：提出一个用户可能会想反驳的判断，而这个判断必须从他自己的梦境细节里长出来，能指回具体是哪个梦的哪一处。用户可以修改或忽略这份画像，所以宁可说错，不要说空。',
    '严禁复用本提示中出现过的任何示例措辞或句式。本提示只描述标准，不提供可以照抄的句子；出现照抄即视为未完成。',
    '重点放在当前阶段：最近正在发生什么、和之前有什么不同，而不是全部历史的汇总。',
    '不诊断、不预测、不断吉凶。不得断言用户的潜意识、动机或任何他没有说出口的心理内容（「你的潜意识在告诉你」这类写法属于诊断）。可以说梦里发生了什么、你观察到什么倾向，不能替用户宣布他内心真正想要什么。',
