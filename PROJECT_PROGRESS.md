...
## 2026-08-05 梦境阐释重排：按「可核对性」决定顺序

- 重新定义共鸣：**共鸣 = 具体到用户能当场核对**。「你渴望被理解又害怕暴露」这类话读的瞬间有感觉、三秒后蒸发，而且永远不可能说错——不可能错的东西，对了也不算数。能被否定，才值得相信。
- 由此得出唯一的排序规则：**越能被用户当场核对的越靠前，越是对谁都成立的越靠后或不显示**。对照这条规则，原有编排几乎完全倒置：可核对性最低的「文化象征」占据可见区第一位，而「你之前提到过」「与你有关」全在默认收起的折叠里。
- 折叠区原本用 `dream_translation`（用户刚写的梦的复述）开场，而原梦全文就在上面两屏。点开折叠的奖励是再读一次自己写的东西——这不只是藏起来，是**教会用户「展开不值得」**，进而把后面的「与你有关」永久埋掉。
- 搬出折叠：「你之前提到过」（引用用户确认过的原话＋日期＋溯源链接，可核对性最高）、「与你有关」、以及那句边界声明「这是一种理解角度，你可以保留、修正或否定它」。
- 边界声明必须可见的理由不是免责，而是**可反驳性本身就是共鸣的一部分**：一份无法反驳的解读就是星座运势。原先它只在折叠内，导致不展开的用户读到的是没有任何边界的断言——最容易过度相信的那批人一个字都看不到。
- 这同时修好了记忆呼应的落点问题：prompt 要求已核实的历史呼应落在 `possible_connections` 或 `underneath`，前者原本在折叠内、后者可见，模型选哪个是抛硬币。两处现在都在折叠外，不会再出现「命中率很高但用户没看见」。
- 搬进折叠：「文化象征」（公共知识，谁做同样的梦都一样），且无依据时允许完全不显示。折叠更名「完整解读」→「再多看一点」——原名暗示正片在里面，既贬低外面又保证打开后失望。
- 零关联现在说得出口：`possible_connections` 与 `mirror` 都为空时显示「这次没有找到和你生活的具体呼应。梦不一定每次都指向什么。」主动说这次没找到，比每次都硬找一个更能让人相信找到的那次。
- 兜底文案去掉「梦里有人说过话」的预设。旧文案在「沙漠里下暴雨」「西红柿里爆出水蜜桃」这类梦上触发时，会对用户自己的梦说假话——别的泛还能忍，说错我的梦立刻就知道在编，这是最伤信任的一类错，也违反红线一。`mirror` 兜底改为留空，与上一条的零状态一致。
- 诊断页新增**折叠区展开率**（按去重梦数计，避免反复展开收起把比例刷过 100%）。这个数要反着读：正片已搬到折叠外，展开率**下降**才是成功；展开率高说明外面留的不够。
- 新增位置断言（`check-miniprogram-mvp`）：直接比较各模块在模板中的偏移量与折叠区起点，三个模块搬回原处会立刻失败。纯字符串存在性检查测不出位置，这次改版的全部意义就在折叠这条线的两侧。
- 未改动：⑨命理视角入口（保持 opt-in）、⑩今天的小行动、⑪反馈在「聊聊这个梦」之前——聊天入口的提问会根据是否已反馈变化，顺序反了联动即失效。折叠里只放内容，不放入口。
- 本地验证通过：`npm run check:mini-release` 全绿、`git diff --check`。
- 待部署：`interpretDream`（兜底文案）、`saveDream`（展开率统计）。

## 2026-08-06 内测反馈第一轮：六个「用不了」的修复

这一轮全部来自真实内测反馈。六条里有五条的共同点是：**功能本身是好的，但失败或
限制发生时页面什么也没说**——用户读到的不是「这次没成功」，而是「这个东西坏了」。

- **语音输入提示失败**。真正的根因在首次使用：长按触发 → `wx.authorize` 弹窗 →
  用户手指抬起 → 授权返回后代码照常 `start()` 再立刻 `stop()`，录到一个 0 长度
  文件，用户看到「语音识别暂不可用」。一次成功的授权被显示成功能坏掉，而这必然
  发生在每个新用户身上。现在授权竞态下不启动录音，只提示「麦克风已开启，再按住
  说一次」；同时新增 `MIN_RECORD_LIFETIME_MS`（600ms，start 后过早的 stop 顺延）
  与 `MIN_RECORD_MS`（1s，过短片段不送 ASR，直接说「太短」而不是「没有听清」）。
  云函数侧自己在 15 秒收口返回 `recognize_timeout`，并把腾讯 ASR 的 `code` 以
  `providerErrorCode` 带回；客户端对网络类失败自动重试一次。
- **不知道要按着圆圈下滑**。一个只能靠手势触发的主流程，对第一次来的人等于没有
  入口。圆环下方的落点条改成真正的 `button`：有草稿时它就是「解读这个梦 →」，
  点一下即可；下滑保留为已学会者的快捷方式，两条路走同一个 `generateDreamCard`。
  两条路径上报同一个 `dream_submit_gesture` 事件、只区分 `via`，「有多少人真的
  学会了下滑」从此是一个可以直接查的数字。
- **梦卡出不来**。生图要求这条梦先在云端存在。预同步失败时代码把 `imageStatus`
  设回 `'idle'`——而 `'idle'` 在 WXML 里既不显示提示、也不显示「重新生成画面」
  按钮。于是生图从未开始这件事对用户完全静默：一张永远停在底色渐变上的卡，且
  没有任何可点的东西。改为可见的 `'failed'` 态 + 自动重试一次（只一次，坏掉时
  不反复烧供应商额度）+ `onShow` 补偿（离开再回来是用户最自然的自救动作）+
  「重试同步」成功后自动接着生图。
- **梦后对话卡死无法发送**。聊满 6 轮后输入框、语音、发送三个控件同时变灰，页面
  上没有任何一句话解释，只剩一个「6 / 6」。轮次上限是刻意的设计，但它必须自己
  说出口并给出出口：现在换成一张说明原因的结束卡 + 两个去处，倒数第二轮起提示
  「还剩最后一轮」。另外聊天此前继承了 `interpretDream` 70 秒的客户端上限——真的
  等满 70 秒，用户早就认定卡死了，现在收到 35 秒，失败文案区分网络与服务。
- **不想一上来看到自己的梦原文**。长梦（超过一行或含换行）在解读页默认只留第一
  行，可展开；短梦不折叠，多一个「展开」只是噪音。
- **分享缩略图截成横的**。微信会话里的转发缩略图固定按 5:4 显示，而 `imageUrl`
  收到的是 900×1200 的竖版梦卡，微信只能居中裁一条横带出来。现在按 5:4
  （1000×800）单独构图渲染缩略图，相册梦卡仍是 3:4，长图仍是 900×2300。

- 回归覆盖：授权竞态不起录音、过短片段不发起云调用、按钮与手势同源提交、空草稿
  点击不静默、预同步失败必须是可见失败态且自动重试只一次、`onShow` 在有云端图时
  走重取而不是生图（否则会永远停在「生成中」）、首次进入 `onShow` 不与 `onReady`
  重复生图、聊天上限有出口、原文折叠、缩略图导出尺寸必须是 5:4。
- 本地验证通过：`npm run check:mini-release` 全绿、`tsc --noEmit` 干净、
  `git diff --check`。
- 待部署：`speechRecognize`（超时收口 + providerErrorCode）。**控制台里
  `speechRecognize` 的平台超时必须高于 15 秒**，见 `docs/CLOUDBASE_DEPLOYMENT.md`。
- 未做（下一轮单独处理）：阶段画像的位置与内容强化。已定方向为提到「梦册」顶部。

### 同日补充：语音状态机重写 + 提交按钮重做

上一条的语音修复只补了失败路径，没有动状态机本身，真机上仍然「有问题」。根因是一条
写在 `onVoiceTouchMove` 里的分支：**手指位移越过 12px slop 就撤掉还没触发的长按计时器**。
它原本是为了让「有草稿的人下滑提交」不误起录音，但代价是**按下后马上开始拖的人根本
录不上音**——按住说话在他手里就是坏的。而这个分支存在的理由，在圆环下面有了可点的
按钮之后已经不成立了。

现在 `pages/home/index.js` 顶部写着这套交互唯一的权威描述：

- 按住圆环 240ms → 进入语音输入，**圆环变黑**，开始录音
- 之后手指拖到哪都保持录音，直到松手：上方区域松手 = 取消整段丢弃；下方区域松手 =
  转文字后直接进入解读；**其余任何位置松手 = 只转成文字**
- 唯一决定录不录的是按住时长，唯一决定收尾方式的是松手那一刻手指在哪。`touchmove`
  不得再碰长按计时器。

配套调整：`is-recording` 改为整圆墨色实心（这是「已进入语音输入」唯一不依赖文字的
信号，且必须在整个拖拽过程中保持不变）；`is-drag-submit` 底色已是黑的，改用一圈旧金
光环区分，不再靠改填充色抢同一个信号；`is-drag-cancel` 转朱砂实心。删掉了「再往下滑
一点」的 attempt 提示——录音已经正常收下并开始识别，此时说「没划到位」会让人以为刚
说的话丢了。录音中的提示行改为一次说全三条出路。

「解读这个梦」按钮重做：胶囊圆角在这套纸墨排版里是异物，是它读起来像个没做完的标签
的主要原因。改成与全站 `.primary-button` 同一套语言（2rpx 直角、墨底纸字、`--fs-aux`
800 字重、400×96rpx）。箭头并进同一个 `text` 节点——拆成两个节点时中文和箭头各按自身
字体算基线，会差半个像素地错开。

- 回归覆盖同步重写：`touchmove` 不得撤销长按计时器、录音中三条出路各自的收尾状态、
  拖了没到位不弹提示也不丢录音、原地松手不弹键盘。
- 顺带修了验收脚本一处会随用例增多而误报的问题：本地事件缓冲上限 120 条，跑完一轮
  完整验收后最早的 `app_start` 会被正常挤出去。断言改为合并 `trackEvent` 云调用记录，
  验证「这些事件确实被埋过」而不是「还留在缓冲区里」。

## 2026-08-06 阶段画像：搬到梦册顶部，并把已存却从未露面的三样东西渲染出来

### 位置

画像原本在「我」页第三屏。「我」是一整页表单——出生日期、性别、保存资料、隐私
说明——把产品里最有辨识度的产出摆在表单上方，它会被读成一个用户自己填的字段，
而不是系统产出的东西。这就是内测反馈说的「违反操作直觉」。

搬到「梦册」顶部。梦册已经有本月观察、重复线索、阶段原型三个同类物件；那三个讲
的是「你的梦里有什么」，画像讲的是「你是怎样的人」，四者合起来才是一段连续的
「你的梦加起来说明了什么」，所以画像排在最前面统领它们。刻意不加第四个 tab：
梦少于 3 个时画像内容单薄，四个 tab 也会稀释「写梦」这个主入口。

「我」页只留一个只读入口（版本 + 状态 + 两行预览 + 「在梦册查看 ›」）。老用户仍能
从熟悉的位置找到它，但不再有两份可编辑的画像 UI。

编排逻辑抽到 `utils/stagePortrait.js`，两页共用一个控制器。两页各自实现会立刻
分叉：同一个快照在两处显示出不同的版本号或状态。

### 内容

三样东西一直存在库里、一次都没渲染过。它们是画像从「好看」变成「可信」的全部差别：

- **溯源**：每个快照存最多 18 条 `sourceRefs`。prompt 明确要求画像的判断「能指回
  具体是哪个梦的哪一处」，界面却不给核对入口——一个无法核对的判断会退化成星座
  运势，正是那份 prompt 一直在对抗的失败模式。现在展开「这段来自你的 N 个梦」
  可以逐条点进那个梦；源梦已删除的仍然列出（画像确实是从它长出来的），但不再
  假装可以点开。
- **历史版本**：`profile_snapshots` 存着每一版，界面只显示当前这一版。
  「三个月前你是这样，现在你是这样」是画像唯一能做、而单次解读永远做不到的事。
- **changeReason**：这一版为什么和上一版不同，每版都存了，从没显示过。

外加一条召回信号：后台每记一个梦就刷新一次画像，此前从不告知任何人。现在版本号
高于用户上次看到的版本时，顶部出现一条可点的「画像更新到 V3 · 看看变了什么 ›」。
首次使用（没有旧版本可对比）不提示。

- 顺手修了一处真实隐患：`archive.onShow` 直接被 `retryArchiveLoad` 调用，而
  `onLoad` 只在页面首建时跑一次，控制器改为懒创建，缺席时不会静默炸掉整页。
- 回归覆盖：画像编辑/迁移/生成失败这一整组用例跟着 UI 搬到 archive 页；新增
  「我」页只剩入口（`startPortraitEdit` 不再存在）、入口仍显示版本与状态、
  历史版本列表按版本号倒序且可展开。
- 本地验证通过：`npm run check:mini-release` 全绿、`tsc --noEmit` 干净、
  `git diff --check`。
- 未做（下一轮）：只在变化实质时才发新版本——现在每记一个梦都重生成，V2 和 V3
  常常只是换了说法，版本号在通胀。这是 `profileMemory` 云函数侧的改动。

## 2026-08-06 阶段画像：只在发生实质变化时才发新版本

上一条把历史时间轴和「画像更新了」放了出来，但它们的价值全部建立在「版本变了 =
真的变了」这个前提上。而 `generate` 此前是无条件发版：每记一个梦画像就 +1，
V2 和 V3 常常只是换了说法。前提一垮，两个功能同时退化成噪音。

### 两道闸

1. **证据指纹**（新增 `sourceFingerprint` 字段）——喂给画像的东西（基础资料、梦、
   生活记录）一个字没变时直接返回当前版本，**连供应商调用都不发起**。指纹刻意
   不含 portrait 那一段：版本号、快照 id、updatedAt 每发一版都会变，带上它们指纹
   就永远不会相等，这道闸等于没有。
2. **结构化断言**（`traits` + `themes` + `realLifeContext` 逐条相同）——证据变了
   （多了一个梦），但模型说的还是同一件事时，就地刷新溯源与指纹，不动版本号。

### 一个被测试抓出来的错误设计

最初把「正文字面相似度 ≥ 0.9」也当成一个独立的抑制条件，是错的，两个方向都会翻车：

- 实测一组中文样本：**同一判断换一遍措辞只有 0.33，完全不同的判断是 0.08**。
  散文相似度在中文上根本区分不开这两者，它承担不了主要判断。
- 反过来，用户手改过画像之后系统会原样保留他的文字，相似度恒为 1——可这时图景
  完全可能已经变了。契约测试正是在这里失败的：新增一个带「水/涨潮」的梦，主题
  真的变了，却因为正文逐字相同而被判成无变化。**「文字没变」和「说的还是同一件
  事」是两回事。**

所以正文相似度降级为兜底，只在旧快照压根没有结构化断言时才用得上。

### 刻意不设中间阈值

两道闸都只在**完全相同**时抑制。失败代价不对称：多一个版本号只是难看，而把一次
真实变化吞掉，用户会觉得画像根本不长进——那正好摧毁了历史时间轴和「画像更新了」
的全部价值。判不准就发版。

### 其他

- 客户端点「重新梳理」传 `force: true`：跳过闸①（用户明确要一次新解读，模型不是
  确定性的），但仍要过闸②。被抑制时给一句回执「这一版和上一版没有实质变化」——
  按钮转一圈回来什么都不说，看起来就是坏了。
- 抑制是 `{ ok: true, unchanged: true }`，客户端按正常结果处理：画像原样保留、
  不进失败态、不显示「已更新」。就地替换当前快照时要求 id 一致，否则旧的那条会被
  挤进历史，凭空多出一版。
- 顺带修好一处：游离旧草稿的归档以前只在发版路径上跑，抑制时草稿会一直挂着被
  `getState` 当成待确认版本。现在三条路径都执行。
- 契约测试里的版本号断言改成相对断言——写死数字会把「这一步发不发版」的契约藏进
  一个常量里，而那正是这轮改动的重点。
- **上线后先看 `profile_portrait_unchanged` 事件的分布**（带 `reason` 和 `forced`）。
  如果 `claims_unchanged` 长期占绝大多数且用户版本号几乎不动，说明闸②过紧，
  要放宽而不是收紧。
- 本地验证通过：`npm run check:mini-release` 全绿、`tsc --noEmit` 干净、
  `git diff --check`。
- 待部署：`profileMemory`。

## 2026-08-06 长语音失败：把音频从请求体里拿出去

反馈是「语音输入时间比较长就会失败」。先算了一遍大小：60 秒 @48kbps 的 base64 只有
469 KB，即使某些机型实际按 192kbps 录也只有 1.83 MB，都远低于云函数 4MB 的闸。
**大小不是天花板，时间才是。**

真正的结构性问题：base64 走 `callFunction` 时，同一段字节要被搬两次——客户端上行
一次（弱网上行常年只有几百 kbps），云函数再原样 POST 给腾讯 ASR 一次。两段耗时都
随时长线性增长，而云函数的平台超时是一堵固定的墙。于是短音频过得去、长音频撞墙，
症状恰好是「越长越容易失败」。调大超时只是把墙往后挪，比例关系不变。

改成：超过 8 秒的录音由客户端 `wx.cloud.uploadFile` 传到 `voice-clips/`，云函数收到
`fileID`，换成签名 URL 后以 `SourceType: 0` 交给腾讯，让腾讯在自己的网络里取音频。
云函数的运行时长从此几乎与音频长度无关。短音频仍走内联，不为一句话多付一次上传
往返；上传失败自动退回内联并在事件里带上 `uploadFallback`，不让用户白说一段话。

原始语音是这个产品里最私密的内容之一：云函数无论识别成败都会 `deleteFile`，客户端
在云函数没跑成时补一次删除。**上线后要在存储控制台确认 `voice-clips/` 长期为空。**

顺带把两个页面各自的 `readFile` + 拼 base64 收进了 `cloudBase.recognizeSpeech`，
之前任何一处改动都得改两遍。失败事件补上 `durationMs` 和 `providerErrorCode`——
「越长越失败」这类问题，只有把时长和失败原因放进同一张表才看得出来。

- 回归覆盖：短音频内联（请求体里有 audioBase64、没有 fileID、不产生上传）、长音频
  走存储（只有 fileID、没有音频字节、cloudPath 落在 voice-clips/）。顺带修好验收
  沙箱里 `readFile` 永远失败的桩——它让内联这条路径此前根本没被测到。
- 本地验证通过：`npm run check:mini-release` 全绿、`tsc --noEmit` 干净。
- 待部署：`speechRecognize`。**平台超时仍须 >15 秒**——这次改动降低了它被撞到的
  概率，但没有取消这个前提。

## 2026-08-06 首页：去掉重复的品牌行，提交按钮改为描边

- 删掉页面内的 `ONEIRO` 品牌行。微信原生导航栏已经用 `navigationBarTitleText`
  显示了同样四个字，两处相隔十几个像素出现同一个词，读起来是渲染重复，还白占
  掉首屏最上面一块留白。`.brand` 的 CSS 一并清掉。
- 「解读这个梦」从 400×96rpx 的深墨实心块改为 300×84rpx 的实线描边。问题不在
  圆角也不在箭头：那个圆环是整页最克制的造型（1rpx 发丝描边 + 8px 圆点），
  下面压一块黑色实心砖，**两者的视觉重量差了一个量级**。改成同族的细线之后，
  按钮和圆环成为一组，而不是一个精致图形被一块广告位坠着。按下的瞬间才填实心
  ——那是唯一需要「按钮感」的时刻。
- 顺手修掉一个看起来像 bug 的现象：原来 `is-actionable` 是墨底、`is-armed` 是
  纸底，有草稿的人往下拖时按钮会在半路从黑翻成白。现在两者都只改描边颜色，
  是一条连续的变化。
- 文案收紧：按钮去掉 `→`（居中按钮里的箭头指向不了任何地方），录音中的
  「下滑到这里 · 直接解读」缩成「下滑到这里」——下方提示行已经把三条路说全了。
- 页面底色未改动。`app.wxss` 本次零改动，`--paper: #f2ecdf` 原样。

## 2026-08-06 语音 60 秒上限：可以停，但不能丢

- 先把事实说清楚：60 秒不是产品定的数，是腾讯云 `SentenceRecognition`（一句话
  识别）接口自身的上限。微信 `recorderManager` 允许到 10 分钟，卡住的从来不是
  客户端。链路上 8 处 60 全是在镜像同一个上限，现在由 `MAX_RECORD_SECONDS`
  统一（首页与梦后对话各一份）。
- 到点时原来的行为是**静默切断**：录音器停、界面退回待机，用户正说到一半，
  屏幕上只有一个数到 60 就消失的秒数。他读到的不是「到上限了」，是「我刚说的
  全没了」——和「录音一长就失败」那条反馈其实是同一种体验。
- 三处改动：
  1. **最后 15 秒改成倒数**，写明「到点这段也会收下」，并在跨过阈值时补一次轻
     震动——说话的人多半没在看屏幕，这是唯一不用抬头就能收到的信号。
  2. **到点那 60 秒照常转成文字进草稿。** 撞上限不等于这段作废；识别结果本来就
     是追加的，用户可以分段把一整个梦说完。
  3. **修掉一个真的会吃掉整段话的状态机漏洞。** 到点时手指还按在圆环上，之后
     那次松手原本会落进 `pendingRecording` 分支，把 `voiceStopAfterAuthorization`
     置位——于是**下一次按住圆环整个变成空按**：用户说了一整段，一个字都进不来；
     若松手落在下滑区，还会在下一次按住时凭空跳进解读。现在到点即把这次手势
     就地作废（`voiceAutoStopped`），松手只是「把手拿开」。
- `stopRecorder()` 新增 `userStoppedRecorder` 标记，用来在 `onRecorderStop` 里
  区分「我们要求停」和「录音器自己到点」。标记在**请求发出的时刻**置位，不是真正
  stop 的时刻——中间还隔着 `MIN_RECORD_LIFETIME_MS` 的延迟。
- 验收侧：沙箱补了可手动「走表」的 `setInterval`（`__tickIntervals`），倒数这类
  跟时间强相关的行为才测得动。另外发现 `speechRecognize` 的 mock 一直落在只有
  `ok: true` 的兜底上、不带 `text`，**每次语音其实都走了失败分支**——「转出来的字
  有没有真的进到草稿里」从来没被验证过，本轮补上。
- `npm run check:mini-release` 全绿。
- 未做（留给下一轮判断）：真要支持 3–5 分钟连续讲述，需要换成腾讯的**录音文件
  识别**（`CreateRecTask` + `DescribeTaskStatus`，异步、支持到小时级）。它同样收
  URL，所以上一轮那条云存储链路已经是它需要的地基；代价是交互从「松手就出文字」
  变成「说完等一会儿」，且这条链路本地 mock 验证不了。
